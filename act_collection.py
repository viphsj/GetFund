...
for cookies in jdCookie.get_cookies():
    ...
    print("##"*30)
    print("【关注商品有礼活动】")
    for collectionActInfo in TopAndNewAct:
        if collectionActInfo["activityTitle"] != "关注商品有礼":
            continue
        skipAdd = False
        url = "https://lzkj-isv.isvjd.com/wxCollectionActivity/activity2/"+collectionActInfo["activityId"]+"?activityId="+collectionActInfo["activityId"]
        ...
        data = {
            'pin': secretPin,
            'activityId': collectionActInfo["activityId"]
        }
        ...
        if response.text == '':
            ...
            skipAdd = True
        if skipAdd == True:
            print(f'{collectionActInfo["shopName"]} {errMsg}\n')
        ...
        print(collectionActInfo["shopName"])
            ...
            for cpvosInfo in cpvosList:
                ...
                if cpvosInfo["collection"] == False:
                    ...
                    if addCart.status_code == 200:
                        ...
                        print(f'关注商品数量 {str(addCart.json()["data"]["hasAddCartSize"])}/{str(needCollectionSize)}')
                    else:
                        continue
        ...
        if getPrize.text.find('"result":true,') > -1:
            ...
            giftMsg += collectionActInfo["shopName"] + " 获得：" + str(getPrize.json()["data"]["name"]) + "\n"
